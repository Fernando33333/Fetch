...
const btnSubmit2 = document.getElementById("idSubmit2")
...
const idContainerFetch = document.getElementById("divPropiedadesFetch")
fetch(`../json/propiedades.json`)
    .then(propiedades => propiedades.json)
    .then(propiedad => {
        idContainerFetch.innerHTML = ""
        propiedad.forEach(idContainerFetch => {
            idContainerFetch.innerHTML += `
         <div class="col-sm-12 col-md-6 col-lg-4 main__col">
             <div id="idCard1" class="card main__card h-100">
                 <img src="./imgs/${propiedad.imagen}" class="card-img-top main__img" alt="..." />
                 <div class="card-body">
                     <p class="card-text main__cardtext">${propiedad.descripcion}
                     </p>
                     <small>${propiedad.valor}</small>
                 </div>
             </div>
         </div>`
        })
...
const inmuebles = async () => {
    let baseDatos = await fetch('../json/propiedades.json')
    let inmuebles = await baseDatos.json()

    return inmuebles
}
inmuebles().then(inmuebles => console.log(inmuebles))
*/
